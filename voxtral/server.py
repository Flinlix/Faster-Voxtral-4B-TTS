...
class TTSRequest(BaseModel):
    ...
    sanitize_text: bool | None = None  # None = use server default (--no-sanitize-text)

    @field_validator("trailing_silence_ms")
    @classmethod
    def cap_trailing_silence(cls, v: int | None) -> int | None:
        if v is not None:
            return min(max(v, 0), 1000)
        return v
...
_orphaned_thread_lock = threading.Lock()
...
async def lifespan(app: FastAPI):
    ...
    logger.info("VoxtralTTS.__init__ done: %.2fs", time.time() - _t0)
    logger.info("Waiting for CUDA graph capture ...")
    _state.tts_engine.wait_for_ready()
    logger.info("Ready. Total startup: %.2fs", time.time() - _t0)
    yield
...
async def _generate_and_stream_audio(
    ...
    def generation_thread() -> None:
        try:
            ...
            logger.warning("Generation thread timed out waiting for consumer: %s", exc)
            generation_error[0] = exc
        finally:
    ...
    try:
        ...
        if generation_error[0] is not None:
            ...
            raise RuntimeError("TTS generation failed") from generation_error[0]
        ...
        if thread.is_alive():
            with _orphaned_thread_lock:
                _orphaned_thread_count[0] += 1
                count = _orphaned_thread_count[0]
            logger.warning(
...
async def create_speech(request: TTSRequest) -> StreamingResponse | JSONResponse:
    ...
    effective_sanitize = request.sanitize_text if request.sanitize_text is not None else _cli_sanitize_text

    # Pre-flight: if sanitization is on, check that something survives the filter.
    # This surfaces the failure as HTTP 400 before the stream starts instead of
    # returning HTTP 200 with zero audio bytes.
    if effective_sanitize:
        cleaned = VoxtralTTS._clean_text(request.input, sanitize=True)
        if not cleaned:
            return JSONResponse(status_code=400, content={
                "error": {
                    "message": (
                        "Input text is empty after applying the Latin-script character filter. "
                        "Set sanitize_text=false to synthesise Arabic, Hindi, or other non-Latin scripts."
                    ),
                    "type": "invalid_request_error",
                },
            })
    ...
    async def locked_stream():
        async with _state.inference_lock:
            async for chunk in _generate_and_stream_audio(
                ...
                sanitize_text=effective_sanitize,
            ):
                yield chunk
...
def main():
    global _cli_device, _cli_quantize, _cli_voice_dir, _cli_compile, _cli_use_cache, _cli_pause_ms, _cli_sanitize_text
    ...
    _cli_pause_ms = min(max(args.pause_ms, 0), 1000)
    _cli_sanitize_text = not args.no_sanitize_text
